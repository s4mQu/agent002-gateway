...
def wav_to_base64(wav: torch.Tensor, sample_rate: int) -> str:
    return base64.b64encode(wav_to_bytes(wav, sample_rate)).decode()


def prepare_request(req: "SynthesizeRequest") -> tuple[dict, list[str]]:
    preset = QUALITY_PRESETS.get(req.quality_preset)
    if not preset:
        raise ValueError(
            f"Unknown quality_preset '{req.quality_preset}'. Valid options: {', '.join(sorted(QUALITY_PRESETS))}"
        )
    ...
    if not chunks:
        raise ValueError("text produced no valid chunks")
...
api = FastAPI(title="Chatterbox TTS Server", lifespan=lifespan)
...
async def synthesize(req: SynthesizeRequest):
    ...
    try:
        params, chunks = prepare_request(req)
...
async def connect(sid, environ):
    logging.info("Socket.IO client connected: %s", sid)


@sio.event
async def disconnect(sid):
    logging.info("Socket.IO client disconnected: %s", sid)


@sio.on("synthesize")
async def synthesize_socket(sid, payload):
    try:
        req = SynthesizeRequest.model_validate(payload)
        if not req.text.strip():
            await sio.emit("synthesis_error", {"detail": "text must not be empty"}, to=sid)
            return
        ...
        async with lock:
            for idx, chunk in enumerate(chunks, start=1):
                await sio.emit("chunk_started", {"index": idx, "text": chunk})
                wav_bytes = await asyncio.get_running_loop().run_in_executor(
                    None,
                    lambda chunk=chunk: generate_chunk_wav(
                        model=model,
                        chunk=chunk,
                        **params,
                    ),
                ...
                await sio.emit("chunk_done", {"index": idx})

        await sio.emit("synthesis_done", {"chunks": len(chunks)})
    except ValidationError as exc:
        await sio.emit("synthesis_error", {"detail": exc.errors()})
    except Exception as exc:
        logging.exception("Socket.IO synthesis failed")
        await sio.emit("synthesis_error", {"detail": str(exc)})
...
def generate_chunk_wav(
    ...
    if wav.dim() == 1:
        wav = wav.unsqueeze(0)
    return wav_to_base64(wav, model.sr)
